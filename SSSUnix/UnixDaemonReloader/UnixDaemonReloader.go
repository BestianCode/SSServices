...
func getFullFileList(conf SBMSystem.ReadJSONConfig, rLog SBMSystem.LogFile) []aList {
	var (
		arr []aList
		tmp aList
	)
...
func checkFCS(conf SBMSystem.ReadJSONConfig, rLog SBMSystem.LogFile) int {
	var (
		get    string
		result = int(0)
		a      aList
...
func cleanFCSdb(conf SBMSystem.ReadJSONConfig, rLog SBMSystem.LogFile) int {
	var (
		//get    string
		//result = int(0)
		a aList
...
func restartFCS(conf SBMSystem.ReadJSONConfig, rLog SBMSystem.LogFile) int {
	var get string
...
func main() {
	...
	const (
		pName = string("SSS Unix Daemon Reloader")
		pVer  = string("1 2015.11.24.23.59")
	)
	...
	for {
		...
		dbase.Init(jsonConfig, `PRAGMA journal_mode=WAL;
